...
JSSExchange* jss_load_from_file(FILE* fp) {
	...
	scanf("%ud %ud\n", n, m);
	data->T = (unsigned int*)malloc(sizeof(unsigned int) * m * n);
	data->P = (unsigned int*)malloc(sizeof(unsigned int) * m * n);
	data->S = (unsigned int*)malloc(sizeof(unsigned int) * m * n); //start
	data->F = (unsigned int*)malloc(sizeof(unsigned int) * m * n); //finish
