...
static unsigned long long get_min_operate(JSSExchange *ex,unsigned long long *O,unsigned int time, unsigned int machine){
    ...
    for(unsigned int i=1;i<=n;i++){
        ...
        if(state == 0 &&(p==1 ||  (GET(O,i,p-1)!=0 && GET(O,i,p-1) <= time))){ //该工序尚未完成且上一道工序在此刻已经完成或者就是第一道工序
            if(cost == 0 || GET(ex->T,machine,i) < cost){
                ...
                result = O((unsigned long long)i,p);
            }
        }
    }
    return result;
...
static bool slove(JSSAlgo *algo,JSSExchange *ex,reporter_func reporter){
    ...
    unsigned long long *O = calloc(sizeof(unsigned long long),m*n);
